...
pub(super) fn sync_orchestrator_skill_in(codex_home: &std::path::Path) -> anyhow::Result<()> {
    ...
    let skill = format!(
        "---\n\
         ...
         4. Call the spawn tool once per agent, in parallel when their tasks are independent; chain them when one needs another's output. For `loom_spawn_agents`, batch up to eight independent tasks in one call and always pass a role: explorer, reviewer, researcher, auditor, planner, spec-writer or triager for read-only work; worker, tester, debugger, migrator or refactorer for implementation work. If another tool schema exposes an agent/role parameter, pass the agent's name there; otherwise name the agent at the start of the task message.\n\
