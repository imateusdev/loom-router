...
fn read_user_sandbox_mode() -> &'static str {
    ...
    let Some(mode) = toml::from_str::<toml::Value>(&raw).ok().and_then(|v| {
        v.get("sandbox_mode")
    ...
    match mode.as_str() {
        ...
        _ => {
            WARNED_INVALID.call_once(|| {
                tracing::warn!(
                    sandbox_mode = %mode,
                    "unrecognized Codex sandbox_mode; defaulting subagent env to danger-full-access"
                );
            });
            "danger-full-access"
        }
