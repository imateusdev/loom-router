...
fn write_merged_catalog(config: &AppConfig, port: u16, native: &Value) -> anyhow::Result<()> {
    ...
    let model_count = catalog
        ...
        .unwrap_or(0);
    if model_count == 0 {
        // An empty catalog leaves the picker unusable. Roll back any managed
    ...
    let block = managed_block(port, config.native_slug_mode, &parsed);
...
fn managed_block(port: u16, native_slug_mode: bool, parsed: &toml::Value) -> String {
